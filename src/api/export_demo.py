...
def export() -> dict[str, int]:
    ...
    # -- genai layer ----------------------------------------------------------
    written["genai"] = len(
        _write("genai", _genai_demo(test, pd_hat, score, decisions)).read_bytes()
    )
...
def _genai_demo(test, pd_hat, score, decisions) -> dict:
    """Worked memo and copilot examples for the dashboard.

    Generated by the same code paths the API uses, so the page shows what the
    system actually produces rather than a hand-written illustration. Without
    credentials these come from the deterministic offline paths, and the page
    says so.
    """
    from src.llm.client import cost_per_1000_decisions, credentials_available
    from src.llm.copilot_agent import ask
    from src.llm.memo_agent import generate
    from src.llm.schemas import MemoInput
    from src.llm.tools import PORTFOLIO_QUERIES

    applicants = _demo_cached()
    memos = []
    for applicant in applicants:
        if applicant["decision"] == "approve" or not applicant["reasonCodes"]:
            continue
        result = generate(
            MemoInput(
                decision_id=str(applicant["skIdCurr"]),
                decision=applicant["decision"],
                score=applicant["score"],
                reason_codes=applicant["reasonCodes"],
                requested_amount=applicant["exposure"],
                stated_income_band=_income_band(applicant["profile"]["income"]),
            )
        )
        memos.append(
            {
                "skIdCurr": applicant["skIdCurr"],
                "decision": applicant["decision"],
                "model": result.model,
    ...
    questions = [
        ...
        "Can we use group-specific thresholds to fix disparate impact?",
        "When must we retrain the model?",
    ]
    answers = []
        ...
        answers.append(
            {
                ...
                "model": result.model,
                "offline": result.offline,
                "toolsCalled": [c["tool"] for c in result.tools_called],
                "costUsd": result.cost_usd,
            }
        )

    return {
        "credentialsConfigured": credentials_available(),
        "unitEconomics": cost_per_1000_decisions(memo_rate=0.40),
        "portfolioQueries": [{"name": k, "description": v} for k, v in PORTFOLIO_QUERIES.items()],
        "memos": memos,
        "copilot": answers,
    }
...
_APPLICANT_CACHE: list[dict] = []


def _demo_cached() -> list[dict]:
    """Reuse the applicants already written this run."""
    if _APPLICANT_CACHE:
        return _APPLICANT_CACHE
    import json as _json

    path = OUT / "applicants.json"
    if path.exists():
        _APPLICANT_CACHE.extend(_json.loads(path.read_text()))
    return _APPLICANT_CACHE
